...
class leaky_relu(object):
    ...
    def backward(self, dprev):
        ...
        # Store the output gradients in the variable dfeat provided above.          #
        #############################################################################
        dfeat = np.where(feat >= 0, dprev, dprev * self.negative_slope)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class dropout(object):
    ...
    def forward(self, feat, is_training=True, seed=None):
        ...
        if is_training == True:
            if self.keep_prob > 0:
                kept = np.where(self.rng.random(feat.shape) > self.keep_prob, 0, 1)
    ...
    def backward(self, dprev):
        ...
        if self.is_training == True:
            if self.keep_prob > 0:
                dfeat = self.kept * dprev * (1 / self.keep_prob)
            else:
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        #############################################################################
        N = logit.shape[0]
        cross_entropy = - np.log(logit[np.arange(N), label])
...
def softmax(feat):
    ...
    scores = exp_scores / np.expand_dims(exp_sum, axis=1)
